...
function initResults() {
  ...
  // Update --val CSS custom property so the track gradient tracks the thumb
  function updateRangeFill(val) {
    rangeInput.style.setProperty('--val', val);
  }
  ...
  // Number input → sync range + fill → recalculate
  ...
  // Range slider → sync number input + fill → recalculate
  rangeInput.addEventListener('input', () => {
    weightInput.value = rangeInput.value;
    updateRangeFill(rangeInput.value);
    calculate();
  });

  // Seed --val on load so initial thumb position renders correctly
  updateRangeFill(rangeInput.value);
...
function applyStageScale() {
  if (window.innerWidth >= 900) {
    stage.style.removeProperty('--specs-scale');
    return;
  }
  const available = window.innerWidth - 40; // 20px padding each side
  const scale = Math.min(available / 700, 0.82);
  stage.style.setProperty('--specs-scale', scale.toFixed(4));
}
applyStageScale();
  ...
  */
  ...
  // after — just recompute scale on resize, orbit keeps running
    window.addEventListener('resize', () => {
      applyStageScale();
    }, { passive: true });
}
...
function initBeforeAfterStack() {
  ...
  /* ── Generate dots dynamically from card count ───────────
     Empties the wrapper and rebuilds one dot per card so
     adding or removing .bas-card in HTML is all that's needed.
  ─────────────────────────────────────────────────────────── */
  var dots = [];
  if (dotsWp) {
    dotsWp.innerHTML = '';
    cards.forEach(function(_, i) {
      ...
      btn.className   = 'bas-dot' + (i === 0 ? ' bas-dot--active' : '');
      btn.setAttribute('data-idx',      String(i));
      btn.setAttribute('role',          'tab');
      btn.setAttribute('aria-label',    'Card ' + (i + 1));
      btn.setAttribute('aria-selected', i === 0 ? 'true' : 'false');
      dotsWp.appendChild(btn);
      dots.push(btn);
    });
  }
  ...
  function applyInitial() {
    ...
    updateDots(order[0]);   /* ← always reflect actual front card */
  }

  /* ── Update pagination dots ───────────────────────────────
     Match each dot by its data-idx attribute so this works
     regardless of DOM order or count.
  ─────────────────────────────────────────────────────────── */
  function updateDots(frontCardIdx) {
    dots.forEach(function(dot) {
      var dotIdx   = parseInt(dot.getAttribute('data-idx'), 10);
      var isActive = (dotIdx === frontCardIdx);
